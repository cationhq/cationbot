...
# The module that contains the functions.
MODULE = "cationbot.cogs.auto_role.core"


@pytest.mark.asyncio
async def test_on_raw_reaction_add_should_call_toggle(
    mocker,
    use_bot,
    use_reaction_event,
):
    ...
    auto_role = AutoRole(bot=use_bot)
    await auto_role.on_raw_reaction_add(use_reaction_event)

    toggle.assert_called_once_with(use_reaction_event)


@pytest.mark.asyncio
async def test_on_raw_reaction_remove_should_call_toggle(
    mocker,
    use_bot,
    use_reaction_event,
):
    ...
    auto_role = AutoRole(bot=use_bot)
    await auto_role.on_raw_reaction_remove(use_reaction_event)
...
async def test_toggle_should_bypass_if_emoji_name_is_not_present_on_available_roles(
    ...
    mocker.patch.dict(f"{MODULE}.language_roles", {})
    mocker.patch.dict(f"{MODULE}.misc_roles", {})

    auto_role = AutoRole(bot=use_bot)
    await auto_role._toggle(use_reaction_event)

    use_bot.fetch_guild.assert_not_called()
...
async def test_toggle_should_bypass_if_reaction_is_not_on_rules_message(
    ...
    mocker.patch(
        ...
        {use_reaction_event.emoji.name: role_id},
    )
